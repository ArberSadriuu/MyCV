...
const About: React.FC = () => {
  ...
  return (
    <section ref={ref} id="about" className={`py-24 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 text-slate-800 ${fadeInClass}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h2 className="text-5xl font-bold mb-6 tracking-wide text-slate-800">{t('about.heading')}</h2>
          <p className="max-w-3xl mx-auto text-slate-600 mb-10 text-lg leading-relaxed">
            {t('about.text')}
          </p>
          <button
            onClick={() => setIsModalOpen(true)}
            className="bg-slate-700 text-white px-8 py-3 rounded-md hover:bg-slate-800 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl text-base font-semibold focus:outline-none focus:ring-4 focus:ring-slate-400/40"
          >
            {t('about.readMore')}
          </button>
        </div>
